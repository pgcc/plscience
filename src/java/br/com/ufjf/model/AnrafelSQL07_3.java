...
/**
 * Class AnrafelSQL07_3 idRationale nameScientist institutionScientist
 * description day month year time minute
 *
 * /*( 7.3 ) SQL para trazer as interações realizadas entre os cientistas
 * SELECT A.`idRationale`, F.`NameScientist` , F.`institutionScientist` ,
 * A.`data` , A.`hora` , A.`message` FROM `Communication` A LEFT OUTER JOIN
 * `Rationale` C ON ( A.`idRationale` = C.`idRationale` ) LEFT OUTER JOIN `What`
 * D ON ( C.`idWhat` = D.`idWhat` ) LEFT OUTER JOIN `User` F ON (
 * A.`idScientist_Emissor` = F.`idScientist` ) WHERE (A.`idRationale` =1)
 *
 ...
 *
 */
public class AnrafelSQL07_3 implements Serializable {
